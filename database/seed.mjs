...
try {
  await pool.query('DELETE FROM historial_transacciones');
  await pool.query('DELETE FROM clientes');

  for (const [nombre, telefono, saldo] of clientes) {
    await pool.query(
  ...
  const [rows] = await pool.query('SELECT id, nombre, telefono, saldo_actual FROM clientes');
  console.table(rows);
  console.log('\n10 clientes insertados correctamente con UTF-8.');
} catch (err) {
